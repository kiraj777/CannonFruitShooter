...
class CannonFruitShooter(Scene):
    ...
    def draw(self):
        ...
        # Draw sky gradient
        top_color, bottom_color = self.get_gradient()
        fill(top_color[0], top_color[1], top_color[2])
        rect(0, 0, self.size.w, self.size.h)

        # Draw clouds
        for cloud in self.clouds:
            tint(1, 1, 1, cloud["opacity"])
            ellipse(cloud["x"] - cloud["radius_x"], cloud["y"] - cloud["radius_y"], cloud["radius_x"] * 2, cloud["radius_y"] * 2)

        # Draw level transition text
        if self.game_state == 'paused':
            tint(0, 0, 0)
            text(f'Level {self.level}', 'Arial', 24, self.size.w / 2, self.size.h / 2)
            text('Tap to Continue', 'Arial', 16, self.size.w / 2, self.size.h / 2 + 30)
            return

        # Draw game over
        if self.game_state == 'over':
            tint(0, 0, 0)
            text('Game Over', 'Arial', 40, self.size.w / 2, self.size.h / 2 - 40)
            text(f'Level: {self.level}', 'Arial', 30, self.size.w / 2, self.size.h / 2 - 5)
            text(f'Final Score: {self.score}', 'Arial', 30, self.size.w / 2, self.size.h / 2 + 30)
            return

        # Draw cannon
        fill(0.33, 0.33, 0.33)
        rect(self.cannon['x'] - self.cannon['width'] / 2, self.cannon['y'] - self.cannon['height'], self.cannon['width'], self.cannon['height'])
        rect(self.cannon['x'] - 5, self.cannon['y'] - self.cannon['height'] - 20, 10, 20)

        # Draw balls
        fill(0, 0, 0)
        for ball in self.balls:
            ellipse(ball['x'] - self.ball_radius, ball['y'] - self.ball_radius, self.ball_radius * 2, self.ball_radius * 2)

        # Draw fruits
        for fruit in self.fruits:
            fill(1, 0.84, 0, 1) if fruit["is_special"] else fill(0, 0, 0)
            text(fruit["emoji"], 'Arial', fruit["radius"] * 2, fruit["x"] - fruit["radius"], fruit["y"] - fruit["radius"])

        # Draw status
        tint(0, 0, 0)
        text(f'Level: {self.level} | Score: {self.score} | Lives: {self.lives}', 'Arial', 14, 10, self.size.h - 20)
